...
export const FETCH_ALL_STARTUPS = defineQuery(`*[_type=="startup"] | order(_createdAt desc)[0]{
  _id,
  }`)
...
export const PLAYLIST_BY_SLUG_QUERY = defineQuery(`*[_type=="playlist" && slug.current == $slug][0]{
  ...
  title,
  slug,
  select[]->{_id,_createdAt,title,slug,author->{_id,name,slug,image,bio},views,description,category,image,pitch}
  }`)
