...
const page = async ({ params }) => {
  ...
  const editorPosts=select
  
  if (!post) {
    return notFound()
  }

  const parsedContent = converter.makeHtml(post?.pitch || '')

  return (
    <>
      <section className="pink_container !bg-primary !min-h-[230px]">
        <p className="tag">{formatDate(post?._createdAt)}</p>
        <h1 className="heading">{post?.title}</h1>
        <p className="sub-heading !max-w-5xl text-justify">{post?.description}</p>
      </section>
        ...
        <div className="space-y-5 mt-10 max-w-4xl mx-auto">
          <div className="flex-between gap-5">
            <Link href={`/user/${post?.author?._id}`} className="flex gap-2 items-center mb-3">
              <Image src={post?.author.image} alt="avatar" width={64} height={70} className="rounded-full drop-shadow-lg" />
              <div>
